...
public abstract class Workload
{
	...
	private volatile AtomicBoolean stopRequested = new AtomicBoolean(false);
       ...
       * 
       * @return false if the bench.workload knows it is done for this thread. Client will terminate the thread. Return true otherwise. Return true for workloads that rely on operationcount. For workloads that read traces from a file, return true when there are more to do, false when you are done.
       */
	 ...
	 return null;
       ...
       */
      public abstract boolean doInsert(Database db, Object threadstate);
       ...
       * @return false if the bench.workload knows it is done for this thread. Client will terminate the thread. Return true otherwise. Return true for workloads that rely on operationcount. For workloads that read traces from a file, return true when there are more to do, false when you are done.
       */
      public abstract boolean doTransaction(Database db, Object threadstate);
